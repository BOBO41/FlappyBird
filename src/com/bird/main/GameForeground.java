...
public class GameForeground {
	private List<Cloud> clouds = new ArrayList<>(); // 用于管理云彩的容器
	...
	public GameForeground() {
		...
		for (int i = 0; i < Constant.CLOUD_IMAGE_COUNT; i++) { // 读入图片资源
	...
	private void cloudLogic() {
		...
		if (System.currentTimeMillis() - time > CLOUD_INTERVAL) {
			...
			// 云朵随机改变方向，
			try {
				if (GameUtil.isInProbability(1, Constant.CLOUD_DIRCHANGE)) { // 新的云彩方向
					int newDir = GameUtil.getRandomNumber(Cloud.DIR_LEFT, Cloud.DIR_RIGHT + 1);
					if (newDir != cloudDir) { // 风向改变，调整云彩状态
						cloudDir = newDir;
						for (int i = 0; i < clouds.size(); i++) { // 遍历容器中的云朵
							Cloud tempCloud = clouds.get(i);
							tempCloud.setDir(newDir);
						}
					}
					System.out.println(cloudDir);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
